...
[Download Raw Datasets](https://drive.google.com/drive/folders/1wpTWGWYK4IRdIe6GIrrcKthjpSM6Lok3?usp=sharing)
...
[Download DRG Datasets for 2011 -2015](https://drive.google.com/drive/folders/1AkKrD2MhIYT0zWCGbci0t1ye_LM39asV?usp=sharing)
...
[See All Data Used](https://drive.google.com/drive/folders/1fAqt_u72TyYLqfuVihmJ6Y0pS1mzFw8U?usp=sharing)
...
top_10_discharges$hover <- with(top_10_discharges, paste(drg, "<br>", total, " discharges"))
...
discharges_h_bar <- plot_ly(top_10_discharges,
                            ...
                            add_trace(x = ~`2012`, name = "2012", marker = list(color = '#1B9CFC')) %>%
                              ...
                              layout(height = 500,
                                     title = 'DRGs with Most Discharges for 2011-15 (Top 10)',
                                     font = t,
                                     xaxis = x, 
                                     yaxis = y,
                                     barmode = 'stack',
...
total_discharges_plot <- plot_geo(drg_state_avg_covered_charges, locationmode = 'USA-states') %>%
  add_trace(
    ...
    color = ~total_discharges, colors = 'Reds'
  ) %>%
...
avg_covered_charges_plot <- plot_geo(drg_state_avg_covered_charges, locationmode = 'USA-states') %>%
  add_trace(
    ...
    color = ~avg_covered_charges, colors = 'Greens'
  ) %>%
...
plot.2015.highest <- plot_ly(highest.2015, x = ~reorder(Provider.Name, -avg_total_payments), y= ~avg_total_payments, type="bar", marker = list(color = '#9EC43C'), name = "Average Total Payments") %>% 
  add_trace(y = ~avg_medicare_payments, name = "Average Medicare Payments", marker = list(color = '#F6AB3B')) %>% 
...
plot.2015.lowest <- plot_ly(lowest.2015, x = ~reorder(Provider.Name, avg_total_payments), y= ~avg_total_payments, type="bar", marker = list(color = '#9EC43C'), name = "Average Total Payments") %>% 
        add_trace(y = ~avg_medicare_payments, name = "Average Medicare Payments", marker = list(color = '#F6AB3B')) %>% 
...
```{r Part Seven: Analyzing Cost Change with Time, results = 'asis', echo=FALSE, message=FALSE, warning=FALSE}

drg_year_breakdown_top100 <- read.csv("../../Data/Prepared/year_cost_breakdown_100.csv")

covered_top100_medicare <- drg_year_breakdown_top100 %>% plot_ly(x = ~avg_covered_charge, 
                                                                    ...
                                                                    type = 'scatter',
                                                                    mode = 'markers') %>% 
                                                                    layout(title = 'Average Medicare Payments vs Covered Charges vs Time for DRG Top 100 Discharges',
                                                                    xaxis = list(type = "log", title="Average Covered Charges($)"), 
                                                                    yaxis =list(title="Average Medicare Payments($)")) %>% 
                                                                    animation_opts(1000, easing = "elastic", redraw = FALSE)
```

`r covered_top100_medicare`

# Medical Department Statistics

Statistics made for the common to understand the DRG Discharge and Medicare Cost Statistics.

```{r Part Eight:Total Discharge Statistics, results = 'asis', echo=FALSE, message=FALSE, warning=FALSE}
...
medical_dept_discharge <- head(medical_dept_discharge, 15)
colnames(medical_dept_discharge) <- c("medical_department","2011", "2012", "2013", "2014", "2015", "total")
f <- list( family = "Open Sans, Sans Serif, monospace", size = 10, color = "#7f7f7f")
...
medical_dept_discharge_h_bar <- plot_ly(medical_dept_discharge,
                            x = ~`2011`,
                            y = ~reorder(medical_department,total), 
                            type = 'bar', 
                            orientation = 'h',
                            name = '2011', marker = list(color = '#ffeb3b')) %>% 
                            add_trace(x = ~`2012`, name = "2012", marker = list(color = '#ffd740')) %>%
                              add_trace(x = ~`2013`, name = "2013", marker = list(color = '#ffc400')) %>% 
                              add_trace(x = ~`2014`, name = "2014", marker = list(color = '#ffab00')) %>% 
                              add_trace(x = ~`2015`, name = "2015", marker = list(color = 'ff6f00')) %>%
                              layout(height = 800, 
                                     title = 'Top 15 Medical Departments with Most Discharges',
                                     font = t,
                                     xaxis = x, 
                                     yaxis = y,
                                     barmode = 'stack',
                                     margin = list(l = 150, pad = 4), 
                                     yaxis = list(tickangle = 90),
                                     legend = list(title = 'Legend',
                                                   font = list( family = "sans-serif",
                                                                size = 8,
...
##############################################################################################
# Cost Break Down Bar Chart                                            #
##############################################################################################
cost_breakdown <- read.csv("../../Data/Prepared/medical_department_breakdown_payments.csv")
cost_breakdown <- cost_breakdown %>% arrange(-discharges)
...
medical_dept_cost_breakdown_chart <- plot_ly(cost_breakdown, 
                                y = ~reorder(medical_department, avg_covered_charge), 
                                x = ~avg_covered_charge, 
                                type = 'bar', 
                                name = 'Average Covered Charges', 
                                marker = list(color = '#31A4DB'),
  ...
  add_trace(x = ~avg_medicare_payment, name = 'Average Medicare Charges', marker = list(color = 'F6AB3B')) %>% 
  layout(height = 800,
         title = "Average Cost Breakdown for the Medical Departments",
         font = t, 
         xaxis = list(title = 'Cost ($)'),
         yaxis = list(title = ''), 
         barmode = 'group', margin = list(l=220, pad = 4), 
         xaxis = list(tickangle = 45),
         legend = list(title = 'Legend',
                       font = list( family = "sans-serif",
                                    size = 10,
                       ...
                       borderwidth = 0.5)) 
```
## Top 15 Medical Departments with Most Discharges
`r medical_dept_discharge_h_bar`

## Top 15 Medical Departments Costs Breakdown
`r medical_dept_cost_breakdown_chart`
